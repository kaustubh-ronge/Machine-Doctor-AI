...
export async function analyzeMachine(formData) {
  ...
  try {
    ...
    // UPDATED: Using 1.5-flash because 2.5 was causing the 503 Overload errors
    const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
    ...
    while (attempt < maxRetries) {
      try {
        ...
        if (isOverloaded && attempt < maxRetries) {
          ...
          await delay(attempt * 1000); // Wait 1s, then 2s
        } else {
          throw error; // If it's not a 503, throw immediately
        }
      }
    }

    // Clean JSON
    // Find the first '{' and last '}' to be safe against Markdown noise
    ...
    // 7. Save to Database (WITH SAFE CHECKS)
    // We use ?. operator and || [] to prevent "Cannot read properties of undefined"
    const newReport = await prisma.report.create({
      data: {
        userId: user.id,
        machineId,
        ...
        // FIXED: Safe access to maintenancePlan
        recommendations: aiData.maintenancePlan?.immediateActions || ["See full report for details."],
